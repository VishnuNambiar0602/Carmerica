...
import { getOptimizedPrice } from '../src/services/PriceOptimizer.js';
import { identifyCarPart } from '../src/services/VisionService.js';
import { getPredictiveMaintenance } from '../src/services/MaintenancePredictor.js';
import { smartSearchGarages } from '../src/services/SmartSearch.js';
import { analyzeReviewIntegrity } from '../src/services/ReviewModerator.js';
 ...
 * Grounded in "AI Marketplace" vision for dynamic vendor pricing.
 */
router.post('/ai/optimize-price', async (req, res) => {
  try {
    const recommendation = await getOptimizedPrice(req.body);
...
/**
 * Visual Part Identification Endpoint (The Eyes)
 * Grounded in "Smart Garage" vision.
 */
router.post('/ai/identify-part', async (req, res) => {
  try {
    const { image, mimeType } = req.body; // Expecting base64 string
    if (!image) return res.status(400).json({ error: 'Image data is required' });
    
    const analysis = await identifyCarPart(image, mimeType || 'image/jpeg');
    res.json(analysis);
  } catch (error) {
    console.error('Vision API Error:', error);
    res.status(500).json({ error: 'Failed to identify car part' });
  }
});

/**
 * Predictive Maintenance Diagnosis
 * Analyzes mileage/history to predict service needs.
 */
router.post('/ai/predict-maintenance', async (req, res) => {
  try {
    const diagnosis = await getPredictiveMaintenance(req.body);
    res.json(diagnosis);
  } catch (error) {
    console.error('Diagnostic API Error:', error);
    res.status(500).json({ error: 'Failed to run predictive diagnosis' });
  }
});

/**
 * Smart Search
 * Translates symptoms into garage results.
 */
router.post('/ai/smart-search', async (req, res) => {
  try {
    const results = await smartSearchGarages(req.body.query);
    res.json(results);
  } catch (error) {
...
/**
 * Review Moderation
 * Detects spam/sentiment mismatch.
 */
router.post('/ai/moderate-review', async (req, res) => {
  try {
    const analysis = await analyzeReviewIntegrity(req.body);
    res.json(analysis);
  } catch (error) {
